...
const TableReact = () => {
  ...
  const [totalPage, setTotalPage] = useState(Math.ceil(productList?.length / rowsLimit));
    ...
    if (column != 'Price') {
      if (sortingColumn?.includes(column) && changeSortingColumn) {
        const sortData = productList
          .slice()
          .sort((a, b) => b[column].toString().localeCompare(a[column].toString()));
        setRowsToShow(sortData.slice(currentPage * rowsLimit, (currentPage + 1) * rowsLimit));
        ...
        const sortData = productList
          .slice()
          .sort((a, b) => a[column].toString().localeCompare(b[column].toString()));
        setRowsToShow(sortData.slice(currentPage * rowsLimit, (currentPage + 1) * rowsLimit));
        if (changeSortingColumn) {
          setProductList(sortData);
      ...
      if (sortingColumn?.includes(column)) {
        ...
        setRowsToShow(sortedProducts.slice(currentPage * rowsLimit, (currentPage + 1) * rowsLimit));
        if (changeSortingColumn) {
          setSortingColumn([]);
          setProductList(sortedProducts);
        }
      ...
      <div className="w-full px-2">
                ...
                <th className="py-3 px-3 text-[#212B36] sm:text-base font-bold whitespace-nowrap group min-w-[156px]">
                  <div className="flex items-center">
                    <svg
                      ...
                      } ${sortingColumn?.includes('Category') ? 'rotate-180' : 'rotate-0'}
           `}
                    ...
                    <span className="cursor-pointer pl-1" onClick={() => sortByColumn('Category')}>
                      Category
                    </span>
                  </div>
                </th>
                <th className="py-3 px-3 flex items-center text-[#212B36] sm:text-base font-bold whitespace-nowrap group min-w-[157px]">
                  ...
                  <span className="cursor-pointer pl-1" onClick={() => sortByColumn('Company')}>
                    Company
                  </span>
                </th>
                <th className="py-3 px-3 text-[#212B36] sm:text-base font-bold whitespace-nowrap group min-w-[197px] ">
                  <div className="flex items-center">
                    <svg
                      className={`w-4 h-4 cursor-pointer ${
                        activeColumn?.includes('Product')
                          ? 'text-black'
                          : 'text-[#BCBDBE] group-hover:text-black rotate-180'
                      } ${sortingColumn?.includes('Product') ? 'rotate-180' : 'rotate-0'} `}
                    ...
                    <span className="cursor-pointer pl-1" onClick={() => sortByColumn('Product')}>
                      Product
                    </span>
                  </div>
                </th>
        ...
        >
          <div className="text-lg">
            ...
            {currentPage == totalPage - 1 ? productList?.length : (currentPage + 1) * rowsLimit} of{' '}
